...
subtest 'Koji Client downloads successfully' => sub {
    ...
    # Our mock client returns only one RPM with the information given by
    # rpm_info.
    #
    my $rpms = [
        {
        ...
        {
            # this one is deliberately false and is not to be retrieved.
            'version' => '0.1',
            'nvr' => $rpm_nvr,
            'release' => '1.fc20',
            'arch' => 'i386',
    ...
    my @files = readdir $dh;
    closedir $dh;
    eq_or_diff (sort \@files, sort [('.', 'packages', $rpm_name, '..')], 'successfully downloaded RPM' );
};
