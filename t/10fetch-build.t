...
ok (defined $build->getXMLRPCClient(), 'XMLRPC client correctly assigned');
...
subtest 'Koji Build buildinfo updated correctly' => sub {
    ...
    ok (defined $build->getBuildInfo(), 'buildinfo is defined when updated');
    eq_or_diff($build->getBuildInfo(), $expected, 'buildinfo can be retrieved');
};
