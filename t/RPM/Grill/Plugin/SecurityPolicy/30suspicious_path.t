...
for my $i (0 .. $#tests) {
    ...
    my $security_plugin = create_fake_security_plugin($test, $temp_subdir);

    $security_plugin->analyze();

    eq_or_diff $security_plugin->{gripes}, get_expected_gripes($test),
        $test->{name};
}

done_testing;

sub create_fake_security_plugin {
...
>> -rw-r--r--  root root 0 /noarch/mypkg/usr/share/test_file4
PATH=/usr/bin:/tmp/bin:/opt/foo/bin
...

{
    ...
    code => 'SuspiciousPath',
    context => {
        excerpt => [ 'PATH=/usr/bin:/tmp/bin:/opt/foo/bin' ],
        path => '/usr/share/test_file4'
    },
    diag => 'Potentially insecure PATH element <tt>/tmp</tt>',
    subpackage => 'mypkg'
}
