...
sub _desktop_file_validate {
    ...
    # There's a tool for this, but we need to run the appropriate one.
    my $tool = 'desktop-file-validate';
    if ( $f->rpm->nvr('release') =~ /\.(el5)/ ) {
        $tool .= "-$1";
    }
...
sub _check_exec {
    ...
    # assume /usr/bin .  See rpmdiff:desktoplint.c:make_absolute_exec_path()
    ...
    # arches in the brew build.  This counts as "not found"
