...
sub _analyze_clamscan {
    ...
    # If clamscan isn't installed, run() will abort with:
    #         Command 'clamscan' not found in /usr/bin, /bin
    # ...which our caller will then, via eval, log as a plugin failure.
    my $d = $self->path;
    ...
    # FIXME: we're not actually checking exit status. Should we?
    ...
    unlink $stdout_file;
    if (open my $stdout_fh, '>', $stdout_file) {
        print $stdout_fh $stdout;
        close $stdout_fh;
    }
...
INFECTED_FILE:
    for my $f (@infected_files) {
        ...
        $f =~ m{^(.*?):\s+(.*)}
            or die;
        my ( $path, $msg ) = ( $1, sanitize_text($2) );

        # First component could also be 'ERROR'. Handle gracefully.
        $path =~ s{^$d/}{}
            or do {
                warn "$ME: WEIRD: Cannot grok clamscan result '$f'; ignoring\n";
                $self->gripe({
                    code    => 'ClamAVinternalerror',
                    diag    => 'Unexpected output line from clamscan',
                    context => { excerpt => "$path: $msg" },
                });
                next INFECTED_FILE;
            };
...
sub _analyze_bdscan {
    ...
    open my $bdscan_fh, '<', $tmpfile
        or do {
            ...
            if ($exit_status) {
                %gripe = (
                    code => 'BdScanFailed',
                    diag => "bdscan exited with error status $exit_status",
                );
    ...
    while (my $line = <$bdscan_fh>) {
        if ($line =~ m{^//\s+Core\s*:\s+(.*)}) {        # bdscan version
            ...
            my ($path, $status) = ($1, sanitize_text($2));
            ...
            if ($status !~ /^ok(\s+.*)?$/) {
                ...
                $gripe{context}{path} = sanitize_text($path);
    ...
    open my $avscan_fh, '<', $tmpfile
        or do {
            ...
            if ($exit_status) {
                %gripe = (
                    code => 'AvScanFailed',
                    diag => "avscan exited with error status $exit_status",
                );
...
=item   BdScanMissingResults

The L<BitDefender antivirus tool|http://www.bitdefender.com/>
claims to have run successfully, but produced no output. This
is unexpected. It should not happen.

=item   AvScan

FIXME: the AvScan test is not in production.

=item   AvScanFailed

FIXME: the AvScan test is not in production.

=item   AvScanMissingResults
